...
# THIS CODE GETS THE DIFFUSION, SOURCE RATE AND BINDING OF CELLS

# PART 1 DIFFUSION -> APPLY MONTE CARLO TO GET DIFFUSION IN SOLUTION
# PART 2 SOURCE -> APPLY FIXED RATE FOR THE SOURCE
# PART 3 BINDING -> DO IRRIVERSIBLE BINDING - k_off DOESN'T EXIST, CELLS CANT LET GO
# PART 4 DIFFUSION AND BINDING

# ASSUMPTIONS:
## PARTICLES/MOLECULES ARE THE SAME SO SAME DIFFUSION CONSTANTS
## ALL CELLS ARE THE SAME SO SAME k_on
...
t_step_in = 4     # TIME-STEP WE ADD CERTAIN NUMBER OF PARTICLES TO SOURCE
rate_in = 4        # RATE WE ADD PARTICLES TO SOURCE BOX
...
##    print(grid[t][0:side])
    ...
    for x in range(side):
        ...
        if grid_p[x] != 0: # NOT EMPTY
            ...
            # BINDING PHASE, k_on -> MONTE CARLO STEP
            k_on = round(np.random.rand(1)[0],4)  # 1 NUMBER ARRAY -> [0] IS POINTER TO FIRST ARRAY ELEMENT

            if k_on >= 0.00 and k_on < 0.50:  # CELL DOESN'T LET ALL PARTICLES IN IT TO MOVE AT ALL
                grid_p[x] += 0
            else:
